			...
			<table id="table" class="table color2">
				...
				<c:forEach items="${pageInfo.list}" var="item" varStatus="status">
					<tr>
						...
						<td>${item.name }</td>
						<td class="txtstyle">${item.title }</td>
						<td>${item.updateTime }</td>
						<td>${item.viewCount }</td>
						<td>${item.placement }</td>
						<td>
						<a href="edit.action?id=${item.id }">
					       <button id="add" class="button indigo radius-3"><span class="icon-colorize1"></span> 编辑</button>
                        </a>
						</td>
					</tr>
				</c:forEach>			    		
			...
			<div class="page">
			 ...
			 <div class="block">
					...
					<div class="main" style="margin-bottom: 150px">
						...
						<select id="type_id2" class="select" style="width: 140px;">
							<c:forEach items="${typelist }" var="type_item" varStatus="status">
							<option value="${type_item.id }" 
							<c:if test="${typeId==type_item.id }">  selected="selected" </c:if>>${type_item.name}</option>
					</c:forEach>
     ...
     javaex.date({
		...
		function search(pageNum){
		 ...
		 var keyword=$("#keyword").val();
			window.location.href = "list_namal.action"
			+"?pageNum="+pageNum
		 	+"&typeId="+typeId
		 	+"&startDate="+startDate
		 	+"&endDate="+endDate
